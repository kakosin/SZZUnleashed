    ...
    for row in csv.reader(csvfile):
        ...
        try:
            if not os.path.exists(local_path) or len(os.listdir(local_path)) == 0:
                try:
                    time.sleep(10)
                    result = subprocess.run(["git", "clone", url_git], timeout=600)
        ...
        try:
            ...
            result = subprocess.run(["python3", "/root/fetch_github_bugs/fetch_github.py", github_owner, github_repo], timeout=600)
            if result.returncode != 0:
                print("Error:", result.returncode)
                raise SystemExit(result.returncode)
            
            print("git_log_to_array.py...")                                                                    
            result = subprocess.run(["python3", "/root/fetch_github_bugs/git_log_to_array.py", "--repo-path", github_repo, "--from-commit", Commit], timeout=600)
            if result.returncode != 0:
                print("Error:", result.returncode)
                raise SystemExit(result.returncode)
            
            print("find_bug_fixes.py...")                    
            result = subprocess.run(["python3", "/root/fetch_github_bugs/find_bug_fixes.py", "--gitlog", "./gitlog.json", "--issue-list", "./fetch_issues", "--gitlog-pattern", '"[Cc]loses #{nbr}\D|#{nbr}\D|[Ff]ixes #{nbr}\D"'], timeout=600)
            if result.returncode != 0:
                print("Error:", result.returncode)
                raise SystemExit(result.returncode)
            
            print("szz_find_bug_introducers-0.1.jar...")
            result = subprocess.run(["java", "-jar", "/root/szz/build/libs/szz_find_bug_introducers-0.1.jar", "-i", "./issue_list.json", "-r", github_repo], timeout=600)
            if result.returncode != 0:
                print("Error:", result.returncode)
                raise SystemExit(result.returncode)
