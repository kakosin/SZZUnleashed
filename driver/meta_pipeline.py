...
while ACTUAL_RESULTS_COUNT < NEEDED_RESULTS_COUNT+1:
    ...
    project_crawler.write_to_csv(repositories)
    ...
    ACTUAL_RESULTS_COUNT = len(next(os.walk(project_cleanup.results_path))[1])
    page+=1
    print(f"Found {ACTUAL_RESULTS_COUNT} coherent results...")

if os.path.exists("results.zip"):
    shutil.rmtree("results.zip")
shutil.make_archive("results", 'zip', project_cleanup.results_path)
